...
void setup() {
PImage img1 = loadImage("i3.jpg");
PImage img2 = loadImage("image1.jpg");
size(800, 600);
img1.resize(400, 0);
//img2.resize(400, 0);
...
PImage window = slide_window(img2, 40, 60, 10, 10);
image(img2, 0, 0);
img3 = grayscale(window);
//image(img3, 0, 0);
img3 = sobel(window);
image(img3, img2.width + 20, 0);
img3.resize(400,0);
hog2 = computeHOG(img3, M, N, n);
 
float cos_similarity = cos_similar(hog1, hog2);

//PImage objd = objectdetection(img3, 40, 60, 10, 10);
//image(objd, img3.width + 20, 0);
...
PImage slide_window(PImage img, int window_width, int window_height, int step_x, int step_y) {
...
PImage objectdetection(PImage img, int window_width, int window_height, int step_x, int step_y) {
  noFill();
  PImage img_ = createImage(img.width, img.height, ARGB);
  img_.loadPixels();
  int count = 1;
  for(int y = 0; y< img.height - window_height; y += step_y) {
    for(int x = 0; x < img.width - window_width; x = x + step_x) {
      PImage segment = img.get(x, y, window_width, window_height);
      float [][] h = computeHOG(segment, n, M, n);
      float sim = cos_similar(h, hog1);
      if (sim > threshold) {
        println("candidate:" + x + " " + y + " " + window_width + " " + sim);
        rect(x, y, window_width, window_height);
        segment.save("segment" + count + ".jpg");
        count++;
      }
    }
  }
